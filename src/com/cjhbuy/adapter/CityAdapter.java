...
import com.cjhbuy.utils.PinyinComparator;
...
public class CityAdapter extends BaseAdapter implements SectionIndexer {
	...
	public CityAdapter(Context context, List<CityItem> cities) {
		this.cities = cities;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.title = (TextView) convertView
					.findViewById(R.id.tv_citiy_title);
		...
		if (position == 0) {
			...
			String lastCatalogString = converterToFirstSpell(name.substring(0,
					1));
	...
	public int getPositionForSection(int section) {
		...
		for (int i = 0; i < cities.size(); i++) {
			String l = converterToFirstSpell(cities.get(i).getName())
					.substring(0, 1);
